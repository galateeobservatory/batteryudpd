    ...
    pub fn get() -> Result<Self, &'static str> {
        ...
        Ok(Self {
            ...
            db_port: env::var("POSTGRES_PORT")
                .map_err(|_| "Config: POSTGRES_PORT not set")?
                .parse()
                .map_err(|_| "Config: POSTGRES_PORT not a number")?,
            db_user: env::var("POSTGRES_USER").map_err(|_| "Config: POSTGRES_USER not set")?,
            db_password: env::var("POSTGRES_PASSWORD")
                .map_err(|_| "Config: POSTGRES_PASSWORD not set")?,
            db_name: env::var("POSTGRES_DB").map_err(|_| "Config: POSTGRES_DB not set")?,
            listen_binding: env::var("LISTEN_BINDING")
                .map_err(|_| "Config: LISTEN_BINDING not set")?,
        })
    }
}
